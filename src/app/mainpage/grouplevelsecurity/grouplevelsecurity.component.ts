...
import { Profiledetails } from '../../datamodel/grouplevelsecurity/profiledetails';
import { ProfileMapping } from '../../datamodel/grouplevelsecurity/profilemapping';
...
@Component({
  selector: 'grouplevelsecurity',
  templateUrl: './grouplevelsecurity.component.html',
  styleUrls: ['./grouplevelsecurity.component.css']
})
export class GroupLevelSecurityComponent implements OnInit, SearchInterface, DialogInterface {
  public screenname = "GroupLevel Security";
  ...
  public profilemapping: ProfileMapping;
  ...
  public checkboxvalue; groupcode; groupname; checked; accesstype; moduleid; accessarray = []; usergroupinfo = []; existprofiletype = false;
  public success = true;
  public grouptype = "T";
  public searchid = 2000;
  ...
  newgrouplevelsecurity() {
    this.dialogmodel.message = "Are You want to create new grouplevel security";
    this.dialogmodel.target = "newgrouplevelsecurity";
    this.dialogmodel.dialogtype = "confirm";
    this.maincomponent.invokedialog(this.dialogmodel);
  }

  getusergroupmasterdetails() {
    if (this.existprofiletype != true) {
      this.ivdsservice.getusergroupmasterdetails()
        .subscribe
        (response => {
          if ((response.rs.usergroupmaster).length != 0) {
            for (var i = 0; i < (response.rs.usergroupmaster).length; i++) {
              this.usergroupinfo[i] = response.rs.usergroupmaster[i].grpcode;
            }
            this.checkuseroccurance(this.usergroupinfo);
          }
        },
        err => { console.log(err) });
    }
  ...
  checkuseroccurance(usergroupinfo) {
    var userarray = usergroupinfo;
    for (var i = 0; i < userarray.length; i++) {
      if (userarray[i] == this.groupcode) {
        this.groupcode = "";
        this.dialogmodel.dialogtype = "alert";
        this.dialogmodel.message = "GroupCode already Exists";
  ...
  savevalidations() {
    ...
    if (this.groupcode == undefined || this.groupcode == "") {
      this.dialogmodel.message = "Kindly input groupcode";
      this.dialogmodel.dialogtype = "alert";
      this.dialogmodel.value = null;
      this.maincomponent.invokedialog(this.dialogmodel);
    }
    else if (this.groupcode.length > 15) {
      this.dialogmodel.message = "GroupCode length should not be greater than 15";
      this.dialogmodel.dialogtype = "alert";
      this.dialogmodel.value = null;
      this.maincomponent.invokedialog(this.dialogmodel);
    }
    else if (this.groupname == undefined || this.groupname == "") {
      this.dialogmodel.message = "Kindly input groupname";
      this.dialogmodel.dialogtype = "alert";
      this.dialogmodel.value = null;
      this.maincomponent.invokedialog(this.dialogmodel);
    }
    else if (this.groupname.length > 50) {
      this.dialogmodel.message = "GroupName length should not be greater than 50";
      this.dialogmodel.dialogtype = "alert";
      this.dialogmodel.value = null;
      this.maincomponent.invokedialog(this.dialogmodel);
    }
    else if (this.grouptype == undefined) {
      this.dialogmodel.message = "Kindly input grouptype";
      this.dialogmodel.dialogtype = "alert";
      this.dialogmodel.value = null;
      this.maincomponent.invokedialog(this.dialogmodel);
    }
    else if (this.groupcode != undefined && this.groupcode != "" && this.groupname != "" && this.groupname != undefined && this.grouptype != undefined && this.edittype != false) {
      this.savemoduledetails("update");
    }
    else if (this.groupcode != undefined && this.groupcode != "" && this.groupname != "" && this.groupname != undefined && this.grouptype != undefined && this.edittype != true) {
      this.savemethod();
    }
  }
  ...
  updateprofiledetail(data) {
    $('.hide-Progress-overlay').addClass('is-visible');
    this.ivdsservice.updateprofilemappingdetails(data)
      .subscribe
      (response => {
  ...
  updategroupmasterdetail() {
    var obj = {
      "usergroupmaster": [
        {
          "grpcode": this.groupcode,
          "grptype": this.grouptype,
          "grpname": this.groupname,
          "dateup": this.datepipe.transform(new Date(), "yyyy-MM-dd hh:mm:ss"),
          "userup": "superuser"
        }
      ]
    }
    this.ivdsservice.updateusergroupmaster(obj)
      .subscribe
      (response => {
        if (response.status == 200) {
          ...
          this.dialogmodel.message = "Successfully Updated";
          this.dialogmodel.dialogtype = "alert";
          this.dialogmodel.target = "savemoduledetails"
          this.dialogmodel.value = null;
          this.maincomponent.invokedialog(this.dialogmodel);
  ...
  savemethod() {
    $('.hide-Progress-overlay').addClass('is-visible');
    this.groupmasterdetails = new GroupMasterdetails();
    this.usergroupmaster = new UserGroupMaster();
    this.groupmasterdetails.grpcode = this.groupcode;
    this.groupmasterdetails.grpname = this.groupname;
    this.groupmasterdetails.grptype = this.grouptype;
    this.groupmasterdetails.datecr = this.datepipe.transform(new Date(), "yyyy-MM-dd hh:mm:ss");
    this.groupmasterdetails.dateup = this.datepipe.transform(new Date(), "yyyy-MM-dd hh:mm:ss");
    this.groupmasterdetails.usercr = "superuser";
    this.groupmasterdetails.userup = "";
    this.groupmasterdetails.status = "A";
    this.usergroupmaster.usergroupmaster.push(this.groupmasterdetails);

    this.ivdsservice.setusergroupmasterdetails(this.usergroupmaster)
      .subscribe
      (response => {
        if (response.status == 200) {
          if (this.usermodule.length != 0) {
            ...
            $('#remove').prop('disabled', true);
            this.loadbutton = true;
            this.dialogmodel.message = "Successfully Saved";
            this.dialogmodel.dialogtype = "alert";
            this.dialogmodel.target = "savemoduledetails"
            this.dialogmodel.value = null;
            this.maincomponent.invokedialog(this.dialogmodel);
  ...
  savemoduledetails(type) {
    this.profilemapping = new ProfileMapping();
    ...
    for (var j = 0; j < modulerecords.length; j++) {
      ...
      this.profiledetails.profileid = this.groupcode;
      this.profiledetails.moduleid = modulerecords[j].moduleid;
      this.profiledetails.parentmodule = modulerecords[j].parentmodule;
      this.profiledetails.grpcode = this.groupcode;
      this.profiledetails.code = "";
      this.profiledetails.rights = "";
      this.profiledetails.type = modulerecords[j].clstype;
      this.profiledetails.datecr = this.datepipe.transform(new Date(), "yyyy-MM-dd hh:mm:ss");
      this.profiledetails.usercr = "Superuser";

      if (this.accessarray.indexOf(modulerecords[j].moduleid) > -1 && modulerecords[j].clstype == "M") {
        this.profiledetails.readrights = this.readaccess;
        this.profiledetails.writerights = this.writeaccess;
        this.profiledetails.deleterights = this.deleteaccess;
      }
      else if (this.accessarray.indexOf(modulerecords[j].moduleid) == -1 && modulerecords[j].clstype == "M") {
        this.profiledetails.readrights = "Y";
        this.profiledetails.writerights = "N";
        this.profiledetails.deleterights = "N";
      }
      else {
        this.profiledetails.readrights = "N";
        this.profiledetails.writerights = "N";
        this.profiledetails.deleterights = "N";
      }
      this.profilemapping.profilemapping.push(this.profiledetails);
    }
    this.readaccess = false; this.writeaccess = false; this.deleteaccess = false;
    if(this.profilemapping.profilemapping.length==0)
    {
      this.profilemapping.mode=true;
      this.profilemapping.groupid=this.groupcode;
    }
      else
      {
        this.profilemapping.mode=false;
        this.profilemapping.groupid=this.groupcode;
      }    
    if (type == "update") {
      console.log(this.profilemapping);
      this.updateprofiledetail(this.profilemapping);
    }

    if (type == "save") {
      this.ivdsservice.setprofilemappingdetails(this.profilemapping)
        .subscribe
        (response => {
          if (response.status == 200) {
            $('.hide-Progress-overlay').removeClass('is-visible');
  ...
  getdialogresult(result) {
    if (result.result == "yes" && result.target == "newgrouplevelsecurity") {
      this.groupcode = "";
      this.groupname = "";
      this.usermodule = [];
      this.completemodule = [];
  ...
  setTwocolSearchresult(result: any) {
    ...
    if (result.id == "searchicon") {
      ...
      this.grouptype = data[2];
      $('input').prop('disabled', true);
      this.edit = false;
      this.existprofiletype = true;
      // alert(this.groupcode);
      this.ivdsservice.getprofilemodules(this.groupcode).subscribe
        (response => {
          if (response.rs.profilemapping.length != 0) {
            $('.hide-Progress-overlay').addClass('is-visible');
            var response = response.rs.profilemapping.map(x => Object.assign({}, x));
            for (var i = 0; i < response.length; i++) {
              response[i].lisublist = [];
              response[i].checked = false;
              response[i].indeterminate = false;
